...
class CurrentPageMenuItem extends React.Component {
  render () {
    ...
    const popupContent = isSyncing
      ...
      : (
        <span>
           <strong>Sync this page</strong>.
           Note: the most recent page periodically auto-synchronises!
        </span>
      );
    ...
    return (
      <Menu.Menu position='left'>
        ...
        <Popup position='bottom center'
         trigger={
           <Menu.Item onClick={sync}>
             <Icon name='refresh' color={isSyncing ? 'green' : 'yellow'}
               size='big' loading={isSyncing}/>
           </Menu.Item>
         }
        ...
        <Popup position='bottom center'
         trigger={
           <Menu.Item onClick={() => copyData(balance, 'Current balance copied!', 'balance')}>
        ...
        <Popup position='bottom center'
         trigger={
           <Menu.Item onClick={() => copyData(page.page.seed, 'Current seed copied!', 'key')}>
...
function copyData (data, message='Copied!', icon='check') {
  copy(data);
  showInfo(<span><Icon name={icon}/>{message}</span>);
}
